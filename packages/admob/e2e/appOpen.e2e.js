...
describe('admob() AppOpenAd', () => {
  before(() => {
    AppOpenAd = jet.require('packages/admob/lib/ads/AppOpenAd');
  });
  ...
  describe('createForAdRequest', () => {
    ...
    it('loads with requestOptions', async () => {
      ...
      const i = AppOpenAd.createForAdRequest(firebase.admob.TestIds.APPOPEN, {
  ...
  describe('onAdEvent', () => {
    it('unsubscribe should prevent events', async () => {
      const spy = sinon.spy();
      const i = AppOpenAd.createForAdRequest('abc');
    ...
    it('errors with an invalid ad unit id', async () => {
      ...
      const i = AppOpenAd.createForAdRequest('123');
